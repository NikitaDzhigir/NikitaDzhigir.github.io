...
```
{% raw %}<p>A value from a dictionary: {{ mydict['key'] }}.</p>
<p>A value from a list: {{ mylist[3] }}.</p>
<p>A value from a list, with a variable index: {{ mylist[myintvar] }}.</p>
<p>A value from an object’s method: {{ myobj.somemethod() }}.</p>{% endraw %}
```
...
```
{% raw %}Hello, {{ name|capitalize }}{% endraw %}
```
...
```
{% raw %}{% macro render_comment(comment) %}
    <li>{{ comment }}</li>
{% endmacro %}
...
```
{% raw %}{% import 'macros.html' as macros %}
<ul>
    {% for comment in comments %}
...
```
{% raw %}<p>The local date and time is {{ moment(current_time).format('LLL') }}.</p>
<p>That was {{ moment(current_time).fromNow(refresh=True) }}</p>{% endraw %}
```
